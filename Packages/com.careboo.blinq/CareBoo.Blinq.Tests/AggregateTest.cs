...
internal class AggregateTest
{
    ...
    public void BlinqShouldEqualLinqNativeSequenceAggregateWithAccumulateAndResult([EnumerableValues] IEnumerable<int> source)
    {
        ...
        var expected = ExceptionOrValue(() => LinqEnumerable.Aggregate<int, long, double>(
            source,
            0,
    ...
    {
        var sequence = InitSequence(source);
        var expected = ExceptionOrValue(() => LinqEnumerable.Aggregate<int, long>(
            source,
            0,
            default(LongSum).Invoke
        ));
    ...
    {
        var sequence = InitSequence(source);
        var expected = ExceptionOrValue(() => LinqEnumerable.Aggregate(
            source,
            default(Sum).Invoke
        ));
